...
if chemin_i1 and chemin_i2:
    ...
    # Effectuer la soustraction des matrices et limiter les valeurs négatives à 0
    resultat_soustraction = np.maximum(matrice1 - matrice2, 0)

    # Sauvegarder l'image résultante dans le même répertoire
    chemin_resultat = os.path.join(os.path.dirname(chemin_i1), 'resultat_soustraction.bmp')
    ecrire_matrice_bmp(chemin_resultat, resultat_soustraction)

    print(f"Soustraction des matrices réussie et enregistrée dans '{chemin_resultat}'")
else:
